...
import {getButtons, getMenu, getTopMenu, getUserInfo, loginByUsername, refeshToken} from '@/api/user'
...
const user = {
  state: {
    ...
    loginInfo: getStore({name: 'loginInfo'}) || {},
  ...
  actions: {
      ...
      return new Promise((resolve, reject) => {
        loginByUsername(userInfo.tenantId, userInfo.username, userInfo.password, userInfo.type).then(res => {
          ...
          getRoleListByIds().then(res => {
            ...
            getUserOrg().then(res => {
              let userOrgData=res.data.data;
              data.userOrgList=userOrgData;
              commit('SET_USERIFNO', data);
              resolve();
            });
    ...
    }, SaveLoginInfo({commit}, userInfo) {//保存登录信息
      return new Promise((resolve, reject) => {
        commit('SET_LOGININFO', userInfo);
        resolve();
      })
    }, ForgetLoginInfo({commit}) {//不保留登录信息
      return new Promise((resolve, reject) => {
